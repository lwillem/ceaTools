...
num_sim <- 100
sim_data <- data.frame(incremental_cost   = c(sample(-50:200,num_sim),sample(150:300,num_sim),sample(400:500,num_sim)),
...
plot_ce_plane <- function(incremental_cost,incremental_effect,course_of_action) {
  ...
  # create plot
  plot(incremental_cost ~ incremental_effect, data = sim_data , col = course_of_action)
  abline(h=1,lty=3)
  abline(v=1,lty=3)
  legend('topleft',levels(course_of_action),col=1:length(levels(course_of_action)),pch=1)
}
